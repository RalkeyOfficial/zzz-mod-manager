...
///   permanently.
///
/// Both are tappable and both open the same dialog: the untracked case is not an
/// error, but it is the case a user *may* want to fix, and giving it no affordance
/// would leave the context menu as the only way in.
///
...
class ModStatusSlot extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Tooltip(
      message: loc.t(
        actionable
            ? 'mods.origin.version_unknown_tooltip'
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          // One footprint for both states, so resolving a mod doesn't reflow the
