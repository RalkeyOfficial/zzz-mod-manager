...
- Mod cards now show how well each mod is set up for update checking, in one mark at the corner of the cover: amber when we know which GameBanana mod it is but not which version you have, a quiet clock when only the install date is recorded, and a quiet dot when it isn't linked to a mod page at all. Nothing is shown once the exact file is known. A new toolbar button lists how many mods still need attention — the amber and the unlinked ones — and filters the library down to them.
...
- That same answer can be given to a whole library at once: with the "needs attention" filter on, one button settles every mod that is linked to a mod page but has no known version, using each mod's install date. It makes no network requests, invents no version numbers, and says up front how many mods it will change and how many it can't touch.
- The app can now check your mods for updates — one mod at a time from its right-click menu, or the whole library from a new toolbar button, and only ever when you press it. Mods with something newer published get a blue mark at the corner of their cover; clicking it says what you have, what has been published since, and when.
...
- Once the update check has found something, the same toolbar button filters the library down to just those mods — so three updates among a hundred and twenty-eight mods are one click away rather than something to scroll for. Pressing it again clears the filter, and "Check again" appears beside "Clear filters" while it's on. Once you've dealt with the last update the filter switches itself off rather than leaving you looking at an empty library.
...
- Install confirmations now say what arrived and nothing else — the auto-tags and the list of fields copied from the mod page are gone — and anything you need to act on (a mod with no `.ini`, a download that turned out to be a patch) arrives as its own warning beside it instead of being buried in the same paragraph.
