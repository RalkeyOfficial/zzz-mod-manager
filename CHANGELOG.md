...
- Installing a patch from the Marketplace now asks where it should go, while you are still looking at it. You can give it a folder of its own — and say which mod it patches, so both are checked for updates from then on — or install it straight **into** a mod you already have, picked from a searchable list that shows each mod's cover image rather than a wall of folder names. That second option writes its files over that mod, putting each one where that mod actually keeps it, and records the patch against it. A copy of the folder is saved first, so it can be put back from the mod's backups. You can also install without answering anything, or decide not to install it at all, since the question comes up before anything is copied.
- If a patch's files don't belong in the mod you picked, it says which of the two reasons it is and installs the patch as its own mod instead of guessing — either none of its files are in that mod, so it's for a different one, or that mod's folder holds two copies of them and needs sorting out first. Nothing is written to the mod you picked either way.
...
- The "this looks like a patch" warning after an install now stays until you dismiss it, instead of disappearing after eight seconds beside the success message, and says that update checking will follow the patch rather than the mod it patches.
