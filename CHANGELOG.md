...
- Mod cards now show whether the mod is ready for update checking: an amber mark when we know which GameBanana mod it is but not which version you have, and a quiet dot when it isn't linked to a mod page at all. Nothing is shown once a mod is sorted out. A new toolbar button lists how many mods still need attention and filters the library down to them.
- Clicking that mark — or "Update tracking…" in a mod's right-click menu — opens a dialog to link a mod to its GameBanana page and say which file you have, searching for the folder name or taking a pasted mod page link, and saying *why* it thinks a particular file is yours rather than just guessing.
