...
3DMigoto перезавантажує моди, коли F10 натиснуто **у вікні самої гри** — він не
слідкує за папкою модів, тому жодна зміна на диску не застосується, поки ця
клавіша не дійде. Менеджер модів робить це за вас: знаходить вікно гри, виводить
його на передній план і натискає F10.
...
Це все налаштування, і на **X11, і на Wayland**. Гра працює через Proton, тому її
вікно є вікном X11 у будь-якій сесії — `ydotool`, група `input` і systemd-сервіс
не потрібні й не допоможуть, бо `ydotool` не вміє знаходити вікна.

Далі переконайтеся, що у `d3dx.ini` вашої папки ZZMI лишилася типова прив'язка
3DMigoto:
...
- **Перезавантажити моди** на вкладці «Моди» натискає F10, коли ви попросите.
- **Автоматичне перезавантаження модів** у налаштуваннях натискає її після
  кожного вмикання й вимикання.

В обох випадках **гра виходить на передній план**, бо клавіша доходить лише до
вікна у фокусі.

### Вирішення проблем з F10

Застосунок називає крок, який не вдався, і те саме потрапляє у файл логів
(Налаштування → Діагностика → Файли логів):

| Що ви бачите | Що робити |
|---|---|
| **Гра не запущена** | Запустіть Zenless Zone Zero і не згортайте її вікно |
| **xdotool не встановлено** | Виконайте команду встановлення вище |
| **Не вдалося надіслати F10** | Гра не вийшла на передній план — перевірте, що вона не згорнута |
| **F10 надіслано**, але в грі нічого не змінилося | Перевірте `reload_fixes = no_modifiers VK_F10` у `d3dx.ini` |

Останній рядок варто прочитати двічі: *F10 надіслано* означає, що клавіша дійшла
до гри, а не що 3DMigoto перезавантажив моди. Ніщо поза грою не бачить, чи це
сталося.
...
1. Перевірте, що xdotool встановлений: `which xdotool` — однаково на X11 і на
   Wayland
2. Перевірте, що вікно гри знаходиться:
   `xdotool search --onlyvisible --name Zenless`
3. Переконайтеся, що вікно гри не згорнуте
4. Прочитайте найновіший файл у Налаштування → Діагностика → Файли логів
