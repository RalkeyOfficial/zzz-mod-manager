...
6. **Додайте піктограму та запис у панелі задач** (необов'язково, але потрібно для Wayland):
```bash
./linux/packaging/install-desktop-entry.sh
```

Композитор Wayland не читає піктограму з самого вікна — він шукає desktop-запис
додатка. Без нього вікно показує типову піктограму й безіменний запис у панелі
задач. Скрипт створює запис у домашньому каталозі, не потребує root і
скасовується через `--uninstall`.

**Додаток не з'явиться у списку програм.** Запис створюється з `NoDisplay=true`:
вікно отримує піктограму, але в меню його немає. Передайте `--menu`, якщо хочете
бачити його у списку. Пакунок AUR встановлює звичайний видимий запис, а portable
tarball містить той самий скрипт у `packaging/`.
