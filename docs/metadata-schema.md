...
| `ingest` | `mode` (`separate`/`combined`), `folders` (archive-relative **basenames**), `sibling_group`, `patch_shaped` (this download **replaces rather than adds** — either it brought `.ini` files and none of the content they reference, or it brought no `.ini` and nothing the library did not already have; see [`applying-updates.md` §2](applying-updates.md#2-patch-detection). Knowable only at install). |
